(() => {
  ...
  const DEFAULT_CONTROL_WIRE_SIZE = 16;
  const DEFAULT_WIRING_STYLE = "series";
  const POWER_WIRE_SIZES = [20, 16, 14, 12];
  const CONTROL_WIRE_SIZES = [16, 18, 22];
  ...
  function normalizeControlWireSize(value) {
    const requested = Number(value);
    if (CONTROL_WIRE_SIZES.includes(requested)) return requested;
  ...
  function controlCableLimitFt(tapeOrId, tapeLengthFt, wireSize = DEFAULT_CONTROL_WIRE_SIZE) {
    ...
    return row?.control?.[normalizeControlWireSize(wireSize)] || 0;
  }
  ...
  function normalizeState(targetState) {
    ...
    targetState.controlWireSize = normalizeControlWireSize(targetState.controlWireSize ?? DEFAULT_CONTROL_WIRE_SIZE);
  ...
  function distanceForSuggestedRun(tape, controllerCurrent, run, controllerTapeLength = run.tapeLength, controlWireSize = DEFAULT_CONTROL_WIRE_SIZE) {
    const resolvedControlWireSize = normalizeControlWireSize(controlWireSize);
    if (run.tapeLength <= 0) {
      return {
        goodTotalPathFt: 0,
        maxTotalPathFt: 0,
        ...
        controlWireSize: resolvedControlWireSize,
    ...
    const tapeDropPerFt = ohmsForWire(resolvedControlWireSize) * branchCurrent;
    ...
    const controlSpecLimitFt = controlCableLimitFt(tape, controllerTapeLength || run.tapeLength, resolvedControlWireSize);
  ...
  function buildRecommendation(inputState) {
    ...
    powerSupplies.forEach((supply) => {
      ...
      supply.controllers.forEach((controller) => {
        controller.runs = controller.runs.map((run) => {
          const guidance = distanceForSuggestedRun(tape, controller.inputCurrent, run, controller.totalTapeLength, inputState.controlWireSize);
  ...
  function wireSummaryHtml(totalLength, lengths, emptyText) {
    ...
    const details = lengths
      .map(
        (item) => `
          <span><strong>${escapeHtml(item.label || `${item.wireSize} AWG`)}</strong>Total wire: ${ft(item.length)}</span>
        `
      )
      .join("");
  ...
  function renderLiveResults(result, recommendation = buildRecommendation(state)) {
    ...
    const metricsHtml = [
      ...
      ["Default control wire", controlCableLabel(recommendation.tape, state.controlWireSize)]
    ]
  ...
  function updateFromEvent(event) {
    ...
    if (path === "controlWireSize") {
      const newSize = state.controlWireSize;
      state.tapeRuns.forEach((run) => {
        run.wireSizeToTapeStart = newSize;
        run.farEndWireSize = newSize;
      });
      state.controllers.forEach((controller) => {
        controller.wireSizeControllerToTapeSplit = newSize;
  ...
  function blankProject() {
    ...
    blank.controlWireSize = DEFAULT_CONTROL_WIRE_SIZE;
